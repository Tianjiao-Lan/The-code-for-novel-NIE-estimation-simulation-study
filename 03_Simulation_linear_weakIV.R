#############################################################################
####This code is used for continuous outcome variable with weak instrument###
#############################################################################
#n=sample siz
...
R_square <- F.statistics*2/(F.statistics*2+n-2-1)#根据F值推断R方
...
sim.res.weakiv.linear <- foreach(i=1:nrow(senario)) %:%
...
stopCluster(clus)
names(sim.res.weakiv.linear) <- paste('senario_',1:45,sep='')
save(sim.res.weakiv.linear,file='Simulation_linear.weakiv.RData')
