#############################################################################
###########This code is used for dichotomy outcome variable##################
#############################################################################
#n=sample siz
...
sim.res.nonlinear <- foreach(i=1:nrow(senario)) %:%
...
stopCluster(clus)
names(sim.res.nonlinear) <- paste('senario_',1:45,sep='')
save(sim.res.nonlinear,file='Simulation_nonlinear.RData')